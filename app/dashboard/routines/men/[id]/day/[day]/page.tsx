...
export default async function MenDayExercisesPage({
  ...
  const exercises = await prisma.routineExercise.findMany({
    ...
    include: {
      exercise: { include: { images: true } },
    },
  });

  // Adaptar al formato snake_case que espera DayExercises
  const exercisesForComponent = exercises.map((re) => ({
    ...re,
    notes: re.notes ?? undefined, // ← agregar
    ...
    exercise: {
      ...re.exercise,
      description: re.exercise.description ?? undefined, // ← agregar
      instructions: re.exercise.instructions ?? undefined, // ← agregar
